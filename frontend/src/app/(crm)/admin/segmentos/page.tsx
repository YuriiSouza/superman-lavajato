...
function CategoryManager() {
  ...
  async function handleCreate() {
    ...
    setSaving(true);
    try {
      await crm.services.categories.create({
        name: newName.trim(),
        requiresVehicle: newRequiresVehicle,
      });
      setNewName("");
      setNewRequiresVehicle(true);
      load();
    } finally {
      setSaving(false);
    }
  ...
  async function handleUpdate(id: string) {
    setSaving(true);
    try {
      await crm.services.categories.update(id, {
        name: editName.trim(),
        requiresVehicle: editRequiresVehicle,
      });
      setEditId(null);
      load();
    } finally {
  ...
  function startEdit(cat: any) {
    ...
    setEditRequiresVehicle(cat.requiresVehicle);
  }
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4 space-y-3">
      ...
      <p className="text-xs text-gray-400 dark:text-gray-500">
        Categorias organizam seus serviços. Marque "requer veículo" quando o
        serviço é para carros, motos etc.
      </p>
      ...
      {loading ? (
        ...
        <div className="space-y-1.5">
          {categories.map((cat) => (
            <div
              key={cat.id}
              className="flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/60"
            >
              {editId === cat.id ? (
                <>
                  <div className="flex-1 space-y-1">
                    ...
                    <label className="flex items-center gap-1.5 text-xs text-gray-600 dark:text-gray-400 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={editRequiresVehicle}
                        onChange={(e) =>
                          setEditRequiresVehicle(e.target.checked)
                        }
                      />
                      Requer veículo
                    </label>
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Clique em um segmento para ver os clientes
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
                      ...
                      {seg.clients.map((c: any) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-400 dark:text-gray-500">
                              {c.phone}
                              {c.daysSince !== undefined &&
                                ` · ${c.daysSince} dias`}
                              {c.avgTicket &&
                                ` · ticket R$ ${Number(c.avgTicket).toFixed(2)}`}
                            </p>
                          </div>
                          {c.phone && (
                            <a
