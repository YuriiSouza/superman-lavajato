...
export default function NovaOSModal({ open, onClose, onSuccess }: Props) {
  ...
  function clearClient() {
    setSelectedClient(null); setClientQuery(''); setClientVehicles([]);
    setSelectedVehicle(null); setNewVehicleMode(false); setShowDrop(false);
  }
  ...
  for (const svc of services) {
    ...
    if (!grouped[key]) grouped[key] = { category: svc.category ?? null, services: [] };
  ...
  const dropdown = showDrop && !selectedClient && dropRect
    ? createPortal(
        ...
        >
          {searching && <p className="text-xs text-gray-400 dark:text-gray-500 px-3 py-2.5">Buscando...</p>}
          ...
          <button
            onMouseDown={() => { setNewClientMode(true); setShowDrop(false); setNewClient({ name: clientQuery, phone: '' }); }}
  ...
  const stepLabel = step === 1 ? 'Serviço' : step === 2 ? (requiresVehicle ? 'Cliente & veículo' : 'Identificação') : 'Detalhes';
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow-lg w-full max-w-xl relative max-h-[92vh] flex flex-col">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700 shrink-0">
            <div className="flex items-center gap-3">
              {step > 1 && (
                <button onClick={() => setStep((s) => (s - 1) as 1 | 2 | 3)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
                  <ChevronLeft size={18} />
                </button>
              )}
              <div>
                <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100">Nova ordem de serviço</h3>
                <p className="text-xs text-gray-400 dark:text-gray-500">Passo {step} de 3 — {stepLabel}</p>
              </div>
            </div>
          ...
          {/* Body */}
          <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4" onScroll={() => setShowDrop(false)}>

            {/* ── STEP 1: Serviço ── */}
            {step === 1 && (
              <div className="space-y-4">
                {groups.length === 0 && (
                  <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-8">Nenhum serviço cadastrado ainda.</p>
                )}
                {groups.map((g) => (
                  <div key={g.category?.id ?? 'sem'}>
                    <div className="flex items-center gap-2 mb-2">
                      {g.category ? <Tag size={13} className="text-blue-400" /> : <Car size={13} className="text-gray-400" />}
                      ...
                      {g.services.map((svc) => (
                        <button key={svc.id} onClick={() => selectService(svc)}
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-sm font-semibold text-blue-600 dark:text-blue-400">R$ {Number(svc.price).toFixed(2)}</span>
                          </div>
                          {svc.description && (
                            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5 truncate">{svc.description}</p>
                          )}
                        </button>
                      ))}
                    </div>
            ...
            {step === 2 && requiresVehicle && (
              <div className="space-y-5">
                  ...
                  {!newClientMode ? (
                    <div>
                      <div className="relative">
                        ...
                        <input ref={inputRef} className={`${inputCls} pl-8 pr-8`}
                          placeholder="Buscar por nome, telefone ou placa..."
                          value={clientQuery}
                          onChange={(e) => { setClientQuery(e.target.value); if (selectedClient) clearClient(); }}
                          onFocus={() => clientResults.length > 0 && !selectedClient && openDropdown()}
                          onBlur={() => setTimeout(() => setShowDrop(false), 150)} />
                        {(clientQuery || selectedClient) && (
                          <button onClick={clearClient} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
                            <X size={14} />
                          </button>
                        )}
                      </div>
                      ...
                      {!showDrop && !selectedClient && (
                        <button onClick={() => { setNewClientMode(true); setNewClient({ name: clientQuery, phone: '' }); }}
                ...
                {selectedClient && (
                  <section>
                    <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Veículo</p>
                    {clientVehicles.length > 0 && (
                      <div className="space-y-1.5 mb-2">
                        {clientVehicles.map((v) => (
                          <button key={v.id} onClick={() => { setSelectedVehicle(v); setNewVehicleMode(false); }}
                    ...
                    {!newVehicleMode ? (
                      <button onClick={() => setNewVehicleMode(true)}
                        ...
                        <Plus size={12} />{clientVehicles.length === 0 ? 'Adicionar veículo' : 'Adicionar outro veículo'}
                      </button>
                    ) : (
                      <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-2 bg-gray-50 dark:bg-gray-800/60">
                        ...
                        <div className="flex gap-2 pt-1">
                          <button onClick={() => { setNewVehicleMode(false); setNewVehicle({ plate: '', model: '', color: '', type: 'SEDAN' }); }}
                            className="flex-1 py-1.5 text-xs border border-gray-300 dark:border-gray-600 rounded-lg text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700">
                            Cancelar
                          </button>
            ...
            {step === 2 && !requiresVehicle && (
              <div className="space-y-4">
                <div>
                  ...
                  <input className={inputCls} placeholder="Ex: Moto Honda CG 2020, Quadriciclo, Tapete 3m²..."
              ...
              <div className="space-y-4">
                {selectedService && (
                  <div className="px-3 py-2 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 text-sm">
                    <span className="text-gray-500 dark:text-gray-400 text-xs">Serviço:</span>{' '}
          ...
          <div className="flex gap-3 px-6 py-4 border-t border-gray-100 dark:border-gray-700 shrink-0">
            ...
            {step === 3 && (
              <button onClick={handleSubmit} disabled={saving || !totalValue}
                className="flex-1 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-60">
