...
function CategorySection({
  ...
  async function handleRemove(id: string) {
    if (
      !confirm("Remover categoria? Os serviços vinculados ficam sem categoria.")
    )
      return;
  ...
  function startEdit(cat: any) {
    ...
    setEditReqV(cat.requiresVehicle);
  }
          ...
          <div className="space-y-2">
            ...
            <label className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400 cursor-pointer select-none">
              <input
                type="checkbox"
                checked={newReqV}
                onChange={(e) => setNewReqV(e.target.checked)}
...
export default function ServicosPage() {
  ...
  for (const s of visible) {
    ...
    if (!grouped[key])
      grouped[key] = {
        label: s.category?.name ?? "Sem categoria",
        services: [],
      };
            ...
            >
              ...
              <div className="divide-y divide-gray-100 dark:divide-gray-700">
                {group.services.map((s) => (
                  <div
                    key={s.id}
                    className={`flex items-center gap-4 px-4 py-3 ${!s.active ? "opacity-50" : ""}`}
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
