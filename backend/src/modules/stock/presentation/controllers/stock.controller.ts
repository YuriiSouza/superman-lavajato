...
export class StockController {
  ...
  @Get("intelligence")
  @ApiOperation({
    summary: "Previsão de esgotamento e saúde do estoque por produto",
  })
  async intelligence() {
    ...
    return products.map((p) => {
      ...
      const openingCount = productCounts.find(
        (c) => new Date(c.countedAt) <= thirtyDaysAgo,
      );
      ...
      let totalUsed30 = 0;
      if (openingCount) {
        totalUsed30 = Math.max(
          0,
          Number(openingCount.quantity) + purchases30 - currentQty,
        );
      }
