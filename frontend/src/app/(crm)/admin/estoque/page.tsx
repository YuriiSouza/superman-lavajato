          ...
          <div className="space-y-1">
            {alerts.map((p: any) => (
              ...
              >
                <span>{p.name}</span>
                <span>
                  {fmt(Number(p.quantity))} {p.unit} (mín:{" "}
                  {fmt(Number(p.minQuantity))})
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700">
        {loading ? (
          ...
          products.map((p: any) => {
            ...
            const sinceCount = intel?.lastCountedAt
              ? daysSince(intel.lastCountedAt)
              : null;
                ...
                <div className="flex items-center gap-3 px-4 py-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-500">
                      ...
                      {Number(p.minQuantity) > 0 && (
                        <span> · mín {fmt(Number(p.minQuantity))}</span>
                      )}
                      {p.costPrice && (
                        <span>
                          {" "}
                      ...
                      {sinceCount !== null && (
                        <span
                          className={
                            sinceCount > 2 ? "text-amber-400" : "text-gray-400"
                          }
                        >
                          {" · contagem "}
                          {sinceCount === 0
                  ...
                  {intel &&
                    ...
                    (() => {
                      ...
                      return (
                        <span
                          className={`text-xs shrink-0 ${color}`}
                          title={`Esgota em ${intel.estimatedStockoutDate}`}
                        >
                      ...
                      {intel?.avgDailyUsage > 0 && (
                        <div className="flex gap-3 text-gray-400 dark:text-gray-500">
                          <span>
                            Consumo:{" "}
                            <strong>
                              {intel.avgDailyUsage} {p.unit}/dia
                            </strong>
                          </span>
                          {intel.estimatedStockoutDate && (
                            <span>
                              ...
                              <strong>
                                {new Date(
                                  intel.estimatedStockoutDate + "T12:00:00",
            ...
            <div className="overflow-y-auto flex-1 divide-y divide-gray-100 dark:divide-gray-700">
              <div className="px-5 py-2 bg-blue-50 dark:bg-blue-900/20">
                <p className="text-xs text-blue-600 dark:text-blue-400">
                  Informe a quantidade física de cada produto agora. Os valores
                  já estão pré-preenchidos com o estoque atual — altere apenas o
                  que mudou.
                </p>
              </div>
              {products.map((p: any) => (
