...
const CATEGORIES = ['PRODUTO','COMBUSTIVEL','MANUTENCAO','SALARIO','ALUGUEL','CONTA','OUTRO'] as const;
...
export class BillsController {
  ...
  @Get('summary')
  @ApiOperation({ summary: 'Resumo financeiro: total pendente, vencido, pago no mês' })
  async summary() {
    const now = new Date();
    ...
    const [pending, overdue, paidThisMonth] = await Promise.all([
      this.prisma.bill.aggregate({ where: { status: 'PENDENTE' }, _sum: { amount: true }, _count: true }),
      this.prisma.bill.aggregate({ where: { status: 'VENCIDO'  }, _sum: { amount: true }, _count: true }),
      this.prisma.bill.aggregate({
        where: { status: 'PAGO', paidAt: { gte: startOfMonth, lte: endOfMonth } },
