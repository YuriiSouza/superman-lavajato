...
const nav = [
  ...
  { label: 'Retenção', items: [
    ...
    { href: '/admin/segmentos',   icon: UsersRound,      label: 'Segmentação',        roles: ['ADMIN'] as Role[] },
  ]},
  ...
  { label: 'Sistema', items: [
    ...
    { href: '/admin/usuarios',    icon: UserCog,         label: 'Usuários',           roles: ['ADMIN', 'CAIXA'] as Role[] },
    { href: '/admin/configuracoes', icon: Settings,      label: 'Configurações',      roles: ['ADMIN', 'CAIXA', 'OPERADOR'] as Role[] },
  ]},
];
...
export default function Sidebar({ onClose }: { onClose?: () => void }) {
  ...
  return (
    ...
    >
      {/* cabeçalho */}
      <div className={`flex items-center border-b border-gray-100 dark:border-gray-700 mb-2 pb-3 ${
        collapsed ? 'justify-center px-0' : 'justify-between px-4'
      }`}>
        {!collapsed && (
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate">Superman CRM</p>
      ...
      {/* rodapé */}
      <div className={`pt-4 border-t border-gray-100 dark:border-gray-700 space-y-1 ${collapsed ? 'px-0' : 'px-4'}`}>
        {!collapsed && session?.user?.name && (
          <div className="mb-2">
            <p className="text-xs font-medium text-gray-700 dark:text-gray-300 truncate">{session.user.name}</p>
            <p className="text-[10px] text-gray-400 dark:text-gray-500">{ROLE_LABEL[role]}</p>
          </div>
        )}
