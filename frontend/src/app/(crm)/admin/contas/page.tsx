...
const inputCls =
  "mt-1 w-full text-sm border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500";
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-3">
            ...
            <p className="text-base font-semibold text-yellow-600 dark:text-yellow-400">
              {fmt(summary.pending.total)}
            </p>
            <p className="text-xs text-gray-400">
              {summary.pending.count} conta(s)
            </p>
          </div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-3">
            ...
            <p className="text-base font-semibold text-green-600 dark:text-green-400">
              {fmt(summary.paidThisMonth.total)}
            </p>
            <p className="text-xs text-gray-400">
              {summary.paidThisMonth.count} conta(s)
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700">
        {loading ? (
          ...
          bills.map((b: any) => {
            ...
            const pct =
              billAmt > 0
                ? Math.min(100, Math.round((reserved / billAmt) * 100))
                : 0;
            return (
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-500">
                      {CATEGORIES[b.category] ?? b.category} · Vence{" "}
                  ...
                  <span
                    className={`flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-medium shrink-0 ${STATUS_COLOR[b.status]}`}
                  >
            ...
            <p className="text-xs text-purple-600 dark:text-purple-400 mb-4">
              ...
              {(reserveMap[reserveTarget.id] ?? 0) > 0 && (
                <span className="ml-1 text-gray-400">
                  ({fmt(reserveMap[reserveTarget.id])} já reservado)
                </span>
              )}
            </p>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm p-5">
            ...
            <form onSubmit={handleCreate} className="space-y-3">
              ...
              <button
                type="submit"
                className="w-full bg-blue-600 hover:bg-blue-700 text-white text-sm py-2 rounded-lg mt-2 transition-colors"
              >
                Salvar
              </button>
            </form>
