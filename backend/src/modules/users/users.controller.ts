...
class UpdateUserDto {
  ...
  @IsOptional() @IsEnum(['ADMIN', 'CAIXA', 'OPERADOR']) role?: 'ADMIN' | 'CAIXA' | 'OPERADOR';
...
export class UsersController {
  ...
  async create(@Request() req: any, @Body() dto: CreateUserDto) {
    if (req.user.role === 'CAIXA' && dto.role === 'ADMIN') {
      throw new ForbiddenException('Caixa não pode criar usuários Administrador.');
    }
  ...
  async update(@Request() req: any, @Param('id') id: string, @Body() dto: UpdateUserDto) {
    if (req.user.role === 'CAIXA') {
      ...
      if (target?.role === 'ADMIN' || dto.role === 'ADMIN') {
        throw new ForbiddenException('Caixa não pode editar ou promover usuários Administrador.');
      }
    }
    const data: any = {};
  ...
  async remove(@Request() req: any, @Param('id') id: string) {
    if (req.user.role === 'CAIXA') {
      ...
      if (target?.role === 'ADMIN') {
        throw new ForbiddenException('Caixa não pode remover usuários Administrador.');
      }
    }
