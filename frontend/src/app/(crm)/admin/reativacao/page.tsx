...
export default function ReativacaoPage() {
  const [tab, setTab] = useState<"queue" | "history">(() => {
    try {
      return (localStorage.getItem("reat_tab") as any) || "queue";
    } catch {
      return "queue";
    }
  });
  ...
  const [days, setDays] = useState<number>(() => {
    try {
      return Number(localStorage.getItem("reat_days")) || 30;
    } catch {
      return 30;
    }
  });
  const [contactFilter, setContactFilter] = useState<ContactFilter>(() => {
    try {
      return (localStorage.getItem("reat_contact") as ContactFilter) || "all";
    } catch {
      return "all";
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem("reat_days", String(days));
    } catch {}
    loadQueue(days);
  }, [days, loadQueue]);
  ...
  useEffect(() => {
    try {
      localStorage.setItem("reat_tab", tab);
    } catch {}
  ...
  useEffect(() => {
    try {
      localStorage.setItem("reat_contact", contactFilter);
    } catch {}
  }, [contactFilter]);
            ...
            <>
              <div className="flex gap-0.5 bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
                {CONTACT_FILTER_OPTS.map((o) => (
                  <button
                    key={o.key}
                    onClick={() => setContactFilter(o.key)}
        ...
        (loading ? (
          ...
          <div className="space-y-3">
            {(data.queue as any[])
              .filter((c) => {
                const alreadySent =
                  (sentMap[c.id] ?? c.lastContact?.sentAt) !== null &&
                  (sentMap[c.id] ?? c.lastContact?.sentAt) !== undefined;
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      {timeSince(log.sentAt)}
                    </p>
                    <p className="text-xs text-gray-400 dark:text-gray-500">
                      {dateLabel(log.sentAt)}
                    </p>
                  </div>
                </div>
              </div>
