...
export class GetFinancialUseCase {
  ...
  async summary(params: SummaryParams) {
    ...
    const byService = orders.reduce(
      (acc: Record<string, { count: number; revenue: number }>, o) => {
  ...
  private async calcCmv(start: Date, end: Date): Promise<number> {
    ...
    for (const product of products) {
      ...
      const openingValue = opening
        ? Number(opening.quantity) * costOf(opening)
        : 0;
  ...
  async dre(months = 6) {
    ...
    const [orders, outflows, bills, allCounts, allEntries, products] =
      await Promise.all([
        this.prisma.serviceOrder.findMany({
    ...
    const cmvForRange = (start: Date, end: Date): number => {
      let cmv = 0;
      for (const product of products) {
        const productCounts = allCounts.filter(
        ...
        const opening = productCounts.find(
          (c) => new Date(c.countedAt) <= start,
        );
        const closing = productCounts.find((c) => new Date(c.countedAt) <= end);
        if (!opening && !closing) continue;
  ...
  async listReservesGrouped() {
    ...
    return [...byBill.values()].map(({ bill, entries }) => {
      ...
      return {
        ...
        remaining:
          Math.round(Math.max(0, billAmount - reservedTotal) * 100) / 100,
        pct:
          billAmount > 0
            ? Math.min(100, Math.round((reservedTotal / billAmount) * 100))
            : 0,
        reserves: entries,
      };
    });
  ...
  async reserveSummary() {
    const all = await this.prisma.financialReserve.findMany({
      select: { amount: true },
    });
